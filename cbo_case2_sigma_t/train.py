...
parser.add_argument('--sigma_start', type=float, default=5, help='Dimension of the problem.')
parser.add_argument('--sigma_method', type=str, default="fixed", help='Dimension of the problem.')
...
def create_params_update(update_fn,
    ...
    def compute_error_all(params: dict, rng: jax.random.PRNGKey) -> jnp.ndarray:
        ...
        # key = key.repeat(N_CBO_sampler, axis=0)
...
coeff["sigma"] = 0*coeff["sigma"]+args.sigma_start

value_save = np.zeros((N_iteration, N_CBO_sampler_per_device ))
params_save = np.zeros((N_iteration, N_CBO_sampler_per_device ))
sigma_save = np.zeros((N_iteration ))
for i in range(N_iteration):
    ...
    if args.sigma_method == "adjust":
        coeff["sigma"] *= 0.99
    sigma_save = coeff["sigma"]
    ...
    value_save[i] = value_test1
    params_save[i] = params[0]["W"][0,:,0,0]
    if i % 10 == 0:
        if args.sigma_method == "adjust":
            np.save(f"{config['save_dir']}/adjust_value.npy", value_save)
            np.save(f"{config['save_dir']}/adjust_sigma.npy", sigma_save)
            np.save(f"{config['save_dir']}/adjust_params.npy", params_save)
        else:
            np.save(f"{config['save_dir']}/{coeff["sigma"][0]}_value.npy", value_save)
            np.save(f"{config['save_dir']}/{coeff["sigma"][0]}_params.npy", params_save)
