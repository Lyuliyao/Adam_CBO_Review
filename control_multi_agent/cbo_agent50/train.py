...
def generate_control_loss(
    ...
    def compute_loss(rng: jax.random.PRNGKey, params: dict) -> Tuple[jnp.ndarray, jnp.ndarray]:
        x = (x_start+ 0.01*jax.random.normal(rng, shape=(dim,)))[None, ...].repeat(N_sample, axis=0)
        ...
        for i in range(N_step):
            ...
            loss += fcn_f(x,m) * dt
            # jax.debug.print("loss t {i} :{y}", i=i, y=loss)
            x = x + 10 * dt * m + 0.02*jnp.sqrt(dt) * jax.random.normal(rng, shape=(N_sample, dim))
        
        loss += fcn_g(x)
        # jax.debug.print("loss terminal :{y}",  y=fcn_g(x))
        return loss
...
for i in range(N_iteration):
    ...
    params_save = jax.tree_map(lambda x: x[0:1,0:1], params)
    if i % 10 == 0:
        np.save(f"{config['save_dir']}/value.npy", value_save)
        np.save(f"{config['save_dir']}/params.npy", params_save)
