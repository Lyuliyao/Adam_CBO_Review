...
def generate_control_loss(
    ...
    def compute_loss(rng: jax.random.PRNGKey, params: dict) -> Tuple[jnp.ndarray, jnp.ndarray]:
        key_reset = jax.random.split(rng, N_sample)
        key_step = jax.random.split(key_reset[0], N_sample)
        obs, state = reset_rng(key_reset, env_params)
        loss = 0.0
        for i in range(env_params.max_steps_in_episode):
            action = apply(params, obs)
            key_step = jax.random.split(key_step[0], N_sample)
            obs, state, reward, done, _ = step_rng(key_step, state, action, env_params)
            loss += -reward
        return loss
